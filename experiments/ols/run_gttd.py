...
def run_experiment(datasets: list[str], horizons: list[int], out_name: str) -> Path:
    ...
    for dataset in datasets:
        for horizon in horizons:
            cfg = build_cfg(dataset, horizon, backbone="OLS")
            print(f"[template-gttd] OLS {dataset} H={horizon}")
            ...
            rows = [
                {"dataset": dataset, "backbone": "OLS", "horizon": horizon, "method": "Zero-Shot", "mse": zero["test_mse"], "mae": zero["test_mae"], "adapt_latency_ms_total": 0.0},
                {"dataset": dataset, "backbone": "OLS", "horizon": horizon, "method": "Template-GTTD", "mse": ours["mse"], "mae": ours["mae"], "adapt_latency_ms_total": adapt_ms},
            ]
            pd.DataFrame(rows).to_csv(csv_path, mode="a", header=not csv_path.exists(), index=False)
            print(f"[template-gttd] OLS {dataset} H={horizon} Zero={zero['test_mse']:.4f} Ours={ours['mse']:.4f}")
    return csv_path
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", type=str, default="gttd_ols.csv")
